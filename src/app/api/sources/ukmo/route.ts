...
export async function GET(request: Request) {
  try {
    ...
    const result = await fetchSingleSource('UK_MET_OFFICE', query.save)

    if (!result.success || !result.data) {
      return NextResponse.json(
        createApiError(result.error || 'Failed to fetch UK Met Office data'),
        { status: 500 }
      )
    }
    ...
    console.error('UKMO API error:', error)
    return NextResponse.json(
